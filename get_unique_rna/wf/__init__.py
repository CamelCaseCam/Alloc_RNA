...
@small_task
def get_RNA(length: int, transcriptome: transcriptome, transcriptome_file: Optional[LatchFile], additional_sequences: "list[str]" = []) -> LatchFile:
    ...
    if transcriptome == transcriptome.none:
        ...
        if transcriptome_file is None:
            ...
            transcriptome_file_path = GetTranscriptomeFromFile(transcriptome_file=Path(transcriptome_file).resolve())
    else:
        # If we are using a pre-loaded transcriptome, we need to get the file path
        transcriptome_file_path = get_transcriptome_file(transcriptome=transcriptome)

    Alloc_cmd = [
        "/root/bin/Alloc_RNA/Alloc_RNA",
        str(length),
        str(transcriptome_file_path),
        str(len(additional_sequences)),
       " ".join("\"{}\"".format(s) for s in additional_sequences)
    ]

    Alloc_cmd = "/root/bin/Alloc_RNA/Alloc_RNA " + str(length) + " " + str(transcriptome_file_path) + " " + str(len(additional_sequences)) + " " + " ".join("\"{}\"".format(s) for s in additional_sequences)

    output = subprocess.run(Alloc_cmd, capture_output=True, shell=True)
    print(output.stdout)
    print(output.stderr)
    ...
    return LatchFile("/root/output", "latch:///get_unique_rna/output.txt")


@workflow
def get_unique_RNA(length: int, transcriptome: transcriptome, transcriptome_file: Optional[LatchFile], additional_sequences: "list[str]" = []) -> LatchFile:
    """Description...
    ...
    """
    return get_RNA(length=length, transcriptome=transcriptome, transcriptome_file=transcriptome_file, additional_sequences=additional_sequences)
